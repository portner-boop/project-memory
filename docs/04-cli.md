...
Если [CodeGraph](07-codegraph.md) установлен, `init` делает за тебя всё, что
иначе пришлось бы делать руками:

```text
codegraph init      строит граф   ← запускается ФОНОМ, первым делом
codegraph install   прописывает MCP для Claude Code и Codex
```

Индексация большого репозитория занимает минуты, а разворачивание структуры —
секунды, поэтому индексация стартует первой и идёт параллельно всему
остальному. По времени она обходится почти в ноль.

Если CodeGraph **не установлен** — `init` печатает команду установки и идёт
дальше. Сам он ничего не ставит: глобальный пакет это решение пользователя,
а исполнять удалённый скрипт через `curl | sh` от чужого имени инструмент не
должен в принципе. Поставишь позже — просто запусти `init` ещё раз.

Подробный разбор первого запуска, включая вывод в терминал —
[07 — Первая установка](07-codegraph.md#первая-установка--внутри-repomind-init).
...
repomind init --codegraph auto|skip   # не трогать CodeGraph, даже если он есть
...
недостающее. Это и есть штатный способ доустановить связку с CodeGraph позже.
