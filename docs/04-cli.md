...
| `repomind do` | выполняет план по шагам, после каждого — тесты | ✅ |
...
Читает раздел «План реализации» из `deep-dive.md` и идёт по нему **по одному
пункту**, после каждого прогоняя команды из `docs/runbook/daily.md`.
...
Упало — **останавливается**. Не чинит молча, не идёт дальше. Отметки о
выполненных пунктах пишутся в `progress.md` по ходу, а не в конце: прервался —
`repomind do` продолжит с того же места.

| Флаг | Смысл |
|---|---|
| `--from <N>` | начать с пункта N |
| `--only <N>` | выполнить ровно один пункт |
| `--no-verify` | не гонять тесты (только когда точно знаешь, зачем) |

Зачем по одному пункту, а не всё сразу: **чтобы поломку было видно сразу и
точно знать, какой шаг её принёс.** Пакетное выполнение экономит минуты и
отнимает часы на разбор, что именно сломалось.
