...
| `repomind do` | выполняет прочитанный план по шагам | ✅ |
| `repomind sync` | обновляет документацию по diff | ✅ |
| `repomind check` | валидация | ❌ |
| `repomind index` | пересобирает `_index.json` | ❌ |

Четыре команды из семи работают без всякого ИИ.

Ни одна из них **не ходит в API и не требует ключей** — умные шаги CLI отдаёт
агенту. Как именно — [08 — Как устроен CLI](08-cli-design.md).
...
repomind do    [<TASK-ID>] [--from <N>] [--only <N>] [--no-verify]
...
### `repomind do` — выполнение плана по шагам

Требует, чтобы план был прочитан человеком. В шапке `deep-dive.md`:

```yaml
reviewed: true
```

Нет — команда не запускается:

```text
План не подтверждён.
Прочитай docs/tasks/DEMO-657/deep-dive.md и поставь reviewed: true.
Не согласен — правь прямо там, пойду по исправленному.
```
...
| Флаг | Смысл |
|---|---|
| `--from <N>` | начать с пункта N |
| `--only <N>` | выполнить ровно один пункт |
| `--skip-review` | не требовать `reviewed: true` |

Тесты и линтеры команда **не запускает** — это твоё решение и твои команды,
они в [`docs/runbook/daily.md`](02-doc-model.md#runbook--инструкции-по-запуску).
