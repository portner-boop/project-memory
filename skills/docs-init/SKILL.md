...
Разворачивает память проекта. Делает то же, что делала бы команда
`repomind init`, пока CLI не написан.
...
3. **Не выдумывай архитектуру.** Каркасы архитектурных документов создаются
   пустыми, с подсказкой что писать. Наполнять их — отдельная работа.
...
## Шаг 7. Собрать `docs/_index.json`
...
- Не наполнять архитектурные документы выдуманным содержимым.
