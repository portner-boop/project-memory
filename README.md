...
flowchart LR
    ...
    D --> V{"тесты<br/>прошли?"}
    V -- "нет" --> D
    V -- "да, шаги остались" --> D
    V -- "да, всё" --> S["sync<br/>обновить доки"]
    S --> C["check<br/>валидация"]
    C --> G["✅ git diff"]
    ...
    style V fill:#f0883e,stroke:#db6d28,color:#fff
    style G fill:#3fb950,stroke:#2ea043,color:#fff
```

Оранжевым — **валидация**, и она в двух местах: тесты проекта после **каждого**
шага плана, и проверка целостности документации в конце. Синим — где работает
модель.
...
| 3️⃣ `do` | **выполняет план по шагам**, после каждого гоняет тесты | 🧠 модель |
| ↻ | упало — останавливается и показывает, что именно | тесты проекта |
...
**Про `do`.** Он делает **один пункт плана за раз** и после каждого прогоняет
команды из твоего же runbook. Смысл — чтобы поломку было видно сразу и точно
знать, какой шаг её принёс. Упало — остановился, показал вывод, спросил. Не
чинит молча и не бежит дальше.
