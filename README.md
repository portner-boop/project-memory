...
flowchart LR
    ...
    P --> R{"👀 ты читаешь<br/>план"}
    R -- "поправил" --> R
    R -- "согласен" --> D["do<br/>по шагам"]
    D --> S["sync<br/>обновить доки"]
    S --> C["check<br/>проверить доки"]
    C --> G["✅ git diff"]

    style P fill:#4a9eff,stroke:#2c7cd6,color:#fff
    style D fill:#4a9eff,stroke:#2c7cd6,color:#fff
    style S fill:#4a9eff,stroke:#2c7cd6,color:#fff
    style R fill:#f0883e,stroke:#db6d28,color:#fff
...
| 👀 | **читаешь план и правишь, если не согласен** | **ты** |
| 3️⃣ `do` | выполняет план по шагам, отмечает прогресс | 🧠 модель |
| 4️⃣ `sync` | `git diff` → какие абзацы доков поправить | 🧠 модель |
| 5️⃣ применение | операции → markdown | скрипт |
| 6️⃣ `check` | битые ссылки на код, дубли ID, забытые файлы | скрипт |

**Про прочитку плана.** `do` не начнёт, пока в шапке `deep-dive.md` не стоит
`reviewed: true`. Это не бюрократия, а арифметика: **прочитать две страницы
разбора — пять минут, выяснить то же самое после реализации — полдня.** Не
согласен с подходом — правишь прямо в файле, агент пойдёт по исправленному.

**Про `do`.** Делает один пункт за раз и отмечает в `progress.md` сразу, а не
в конце: прервался — продолжит с того же места. Понял, что пункт неверен —
...
Сначала — **установка инструмента**, это разовое действие, не шаг работы:

| | Сегодня | Потом |
|---|---|---|
| 🔧 Поставить | `./skills/install.sh <проект>` | `pipx install repomind` |

Дальше — рабочие команды. Один в один: **каждому скиллу соответствует ровно
одна команда CLI**, названия совпадают.

| Что нужно | Сегодня — скилл | Потом — CLI |
|---|---|---|
| 📁 Развернуть структуру | `/docs-init` | `repomind init` |
| 📥 Завести задачу из ТЗ | `/docs-task` | `repomind task` |
| 🔍 Разбор + оценка | `/docs-plan ABC-123` | `repomind plan` |
| 👀 Прочитать план | *открыть `deep-dive.md`, поставить `reviewed: true`* | |
| ⚙️ Выполнить план по шагам | `/docs-do` | `repomind do` |
| 🔄 Обновить документацию | `/docs-sync` | `repomind sync` |
| ✅ Проверить целостность | `/docs-check` | `repomind check` |

`repomind index` (пересборка каталога) отдельной командой есть, но руками
не нужна — она вызывается внутри `check` и `sync`.
...
| 🟡 CLI `repomind` | не написан — [устройство расписано](docs/08-cli-design.md) |
| ⚪ Хуки, CI, Jira | спроектированы, включаются позже |

**CLI не будет ходить в API и не потребует ключей.** Он собирает контекст и
применяет результат, а думает агент — той подпиской, которая у тебя уже есть.
Отсюда и один CLI на все агенты: под нового пишется только `SKILL.md`, код не
трогается. Оценка работы — около двух недель, больше половины это файлы и
текст, без всякого ИИ. → [docs/08-cli-design.md](docs/08-cli-design.md)
...
| [08 — Как устроен CLI](docs/08-cli-design.md) | **сложно ли написать, как работает с моделью** |
