...
получаться** после `/docs-init` на реальном проекте.
...
его писать. **Этот файл создаёт `/docs-init`**, вместе с
...
| Итог | [result.md](docs/tasks/DEMO-657/result.md) | дописан скиллом `/docs-sync` |
...
### 6. Инструкции по запуску и подсказки

[`docs/runbook/`](docs/runbook/) — то, что обычно живёт в головах и в чате:

| Файл | Что внутри |
|---|---|
| [setup.md](docs/runbook/setup.md) | поднять с нуля за 20 минут, включая грабли с Node 18 |
| [daily.md](docs/runbook/daily.md) | тесты, линтеры, миграции — **агент берёт команды отсюда** |
| [troubleshooting.md](docs/runbook/troubleshooting.md) | симптом → причина → что делать |
| [release.md](docs/runbook/release.md) | порядок выкатки трёх сервисов и откат |

Обрати внимание на заголовки в troubleshooting: «PDF не генерируется, backend
отдаёт 502» — это **симптом**, а не название проблемы. Человек ищет по тому,
что видит на экране.

И на то, чего в `AGENTS.md` нет: там только ссылка на runbook и три ловушки.
Всё подробное подтягивается по надобности, а не висит в контексте всегда.

### 7. Куда кидать ТЗ

[`docs/requirements/inbox/`](docs/requirements/inbox/README.md) — перетащил
`.docx`, набрал `/docs-task`, всё. Без аргументов и путей.
...
видит их через ссылку. Ссылки создаёт установщик; на Windows — копирует папки.

## Чего в примере нет

**GitHub Actions.** Сознательно: CI не входит в первую версию, чтобы не
усложнять старт. Добавляется в любой момент — см. [05-roadmap](../../docs/05-roadmap.md#ci--можно-включить-в-любой-момент).
